...
disp('Error cometido:')
err = abs(x(length(x)) - x_exact(length(t_exact)))

% Visualización
figure;
plot(t,real(x));
hold on;
plot(t_exact, x_exact)
hold off;
title(['Solución numérica (a = ', num2str(a), ', h = ', num2str(h), ') | Err(1.5) = ', num2str(err)]);
